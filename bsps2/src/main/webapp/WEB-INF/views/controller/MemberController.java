        ...
        try {
            ...
            case "/member/view.do":
                request.setAttribute("vo",
                    Execute.execute(Init.getService(uri), loginId));
                return "member/view";

            // 회원정보 수정 폼
            case "/member/editForm.do":
                request.setAttribute("vo",
                    Execute.execute(Init.getService("/member/view.do"), loginId));
            ...
            case "/member/changeStatus.do":
                ...
                result = (Integer) Execute.execute(Init.getService(uri), vo);
                if (result == 1) {
                    session.setAttribute("msg", vo.getId() + "의 상태가 "
                        + vo.getStatus() + "으로 변경되었습니다.");
                } else {
