...
.btn-save {
	...
	color: white;
	box-shadow: 0 4px 12px rgba(26, 42, 78, 0.15);
}
	...
	<div class="container">
		<aside>
			...
			<ul class="menu-list">
				...
				<li class="menu-item danger"><a href="deleteForm.do"
					onclick="return confirm('정말로 탈퇴하시겠습니까?');">회원 탈퇴</a></li>
			</ul>
		</aside>
			...
			<form action="edit.do" method="post" id="updateForm">
				<div class="form-group">
					<label>아이디</label> <input type="text" name="id"
						class="form-control" value="${vo.id}"
						${login.gradeNo == 9 ? 'readonly' : ''}>
				</div>

				<div class="form-group">
					<label for="name">이름</label> <input type="text" id="name"
						name="name" class="form-control" value="${vo.name}" required>
				</div>
				...
				<div class="form-group">
					...
					<div class="gender-wrap">
						<label class="gender-option"> <input type="radio"
							name="gender" value="남자" ${vo.gender == '남자' ? 'checked' : ''}>
							<span>남자</span>
						</label> <label class="gender-option"> <input type="radio"
							name="gender" value="여자" ${vo.gender == '여자' ? 'checked' : ''}>
							<span>여자</span>
						</label>
					</div>
				</div>
				...
				<div class="form-group">
					<label for="birth">생년월일</label> <input type="date" id="birth"
						name="birth" class="form-control" value="${vo.birth}" required>
				</div>

				<div class="form-group">
					<label for="tel">연락처</label> <input type="tel" id="tel" name="tel"
						class="form-control" value="${vo.tel}" placeholder="010-0000-0000">
				</div>

				<div class="form-group">
					<label for="email">이메일</label> <input type="email" id="email"
						name="email" class="form-control" value="${vo.email}" required>
				</div>

				<div class="button-group">
					<button type="button" class="btn btn-cancel"
						onclick="location.href='view.do'">취소</button>
